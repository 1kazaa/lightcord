/*
 * LIGHTCORD — MultiInstance native.ts
 *
...
import { registerMediaPermissionsForSession } from "../../LIGHTCORD/main/mediaPermissions";
...
function createTokenPreload(token: string): string {
    ...
    const dir = join(app.getPath("userData"), "LIGHTCORD-mi-preloads");
    mkdirSync(dir, { recursive: true });

    const safeToken = JSON.stringify(token); // échappe proprement le token

    const script = `
// LIGHTCORD MultiInstance — token preload
// S'exécute dans le main world AVANT Discord
(function() {
    const TOKEN = ${safeToken};
    try {
        // Définit le token dans localStorage
        Object.defineProperty(window, '__LIGHTCORD_token', { value: TOKEN, writable: false });
        ...
        console.log("[LIGHTCORDMI] Token preload active ✓");
    } catch(e) {
        console.warn("[LIGHTCORDMI] Preload error:", e);
    }
})();
`;
...
export async function openInstanceWindow(
    ...
    try {
        ...
        const uniqueAppId = `LIGHTCORD.instance.${userId}.${Date.now()}`;
        ...
        const partition = `persist:LIGHTCORD-mi-${userId}`;
        ...
        const win = new BrowserWindow({
            ...
            title: `LIGHTCORD [${username || userId}]`,
        ...
        if (process.platform === "win32") {
            try {
                win.setAppDetails({
                    ...
                    relaunchDisplayName: `LIGHTCORD [${username || userId}]`,
                });
            } catch (err) {
                console.warn("[LIGHTCORDMI] setAppDetails failed:", err);
            }
        }
            ...
            wc.executeJavaScript(`
                (async () => {
                    ...
                    try {
                        ...
                        const ws = window.__LIGHTCORD_GW_WS__;
...
// ─────────────────────────────────────────────────────────────────────────────
// Fenetres « groupées » — meme groupe que LIGHTCORD dans la barre des taches
// Principe : on ne touche PAS a setAppDetails => la fenetre herite de l'AppId
// du processus principal (com.LIGHTCORD.app), Windows la groupe automatiquement
// ─────────────────────────────────────────────────────────────────────────────
...
export async function arrangeSplit(_: any, userId: string): Promise<void> {
    try {
        ...
        console.error("[LIGHTCORDMI] arrangeSplit error:", e);
    }
}
