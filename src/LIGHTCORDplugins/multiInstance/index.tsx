/*
 * LIGHTCORD — MultiInstance plugin
 *
...
import { t } from "../autoTranslateLIGHTCORD";
import "./styles.css";
...
const MI_TOKEN_CACHE_KEY = "LIGHTCORD-mi-token-cache";
...
function ContextMenuPortal(props: CtxMenuProps) {
    ...
    const [container] = React.useState(() => {
        const el = document.getElementById("LIGHTCORD-mi-ctx-root") ?? document.createElement("div");
        el.id = "LIGHTCORD-mi-ctx-root";
...
export default definePlugin({
    ...
    async start() {
        ...
        captureCurrentToken();
        addHeaderBarButton("LIGHTCORD-multi-instance", () => <MultiInstanceButton />, 9);
    },

    stop() {
        removeHeaderBarButton("LIGHTCORD-multi-instance");
        if (this._fluxUnsub) { this._fluxUnsub(); this._fluxUnsub = null; }
        const root = document.getElementById("LIGHTCORD-mi-ctx-root");
        if (root) root.remove();
    },
});
