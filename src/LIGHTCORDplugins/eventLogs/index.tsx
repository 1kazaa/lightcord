...
import { t, useTranslation } from "../autoTranslateLIGHTCORD";
import "./styles.css";
...
function loadPersistLogs() {
    try {
        const s = localStorage.getItem("LIGHTCORD_logs");
        if (s) {
...
function savePersistLogs() {
    try {
        ...
        localStorage.setItem("LIGHTCORD_logs", JSON.stringify(toSave));
    } catch { }
}
...
function LogRow({ e }: { e: LogEntry; }) {
    ...
    const onDoubleClick = () => {
        ...
        try {
            ...
            console.error("LIGHTCORD Navigation Error:", err);
...
function LogsModal({ rootProps }: { rootProps: any; }) {
    ...
    const saveAsTxt = useCallback(() => {
        try {
            ...
            a.href = url;
            a.download = `LIGHTCORD_logs_${new Date().toISOString().slice(0, 10)}.txt`;
            a.click();
...
export default definePlugin({
    ...
    start() {
        ...
        loadPersistLogs();
        addHeaderBarButton("LIGHTCORD-event-logs", () => <LogsButton />, 7);
        subscribeToEvents();
    },
    stop() {
        removeHeaderBarButton("LIGHTCORD-event-logs");
