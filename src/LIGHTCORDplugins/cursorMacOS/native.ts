/*
 * LIGHTCORD – CursorMacOS native module
...
const CURSOR_DIR = path.join(app.getPath("userData"), "LIGHTCORDCursors");
...
export async function applyCursors(_: any, style: string, size: string): Promise<{ ok: boolean; error?: string; }> {
    try {
        ...
        if (!macDir) {
            return { ok: false, error: "mac/ directory not found. Place it next to the LIGHTCORD executable." };
        }
        ...
        try {
            execSync(
                `reg add "HKCU\\Control Panel\\Cursors" /ve /t REG_SZ /d "LIGHTCORD macOS" /f`,
                {
                    encoding: "utf-8",
                    windowsHide: true,
                    stdio: "ignore"
