...
export default definePlugin({
    name: "UserAreaTweaks",
    description: "Creates an elegant floating dock for LIGHTCORD plugins using pure CSS, preventing overlap and crashes.",
    authors: [{ name: "LIGHTCORD", id: 0n }],
    enabledByDefault: true,

    start() {
        const style = document.createElement("style");
        style.id = "LIGHTCORD-userarea-tweaks-style";
        style.textContent = CSS;
        document.head.appendChild(style);
    },

    stop() {
        document.getElementById("LIGHTCORD-userarea-tweaks-style")?.remove();
    }
});
