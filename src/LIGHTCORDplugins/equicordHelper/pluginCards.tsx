...
import { EQUIBOT_USER_ID, LIGHTCORD_BOT_USER_ID } from "@utils/constants";
...
const TUTORIALS_API_URL = "https://api.github.com/repos/LIGHTCORDoff/LIGHTCORD-tutorials/contents/videos";
...
export function ChatPluginCard({ url, description }: { url: string, description: string; }) {
    ...
    if (excludedPlugin || !p) {
        const toolTipText = excludedPlugin
            ...
            : "This plugin is not on this version of LIGHTCORD. Try updating!";
...
export const PluginCards = ErrorBoundary.wrap(function PluginCards({ message }: { message: Message; }) {
    ...
    // Process components — LIGHTCORD Bot (LIGHTCORD.online, Component v2 Container format)
    if (message.author.id === LIGHTCORD_BOT_USER_ID) {
        ...
        if (containerComponents?.length >= 3) {
            ...
            if (pluginUrl?.startsWith("https://LIGHTCORD.online/plugins/")) {
