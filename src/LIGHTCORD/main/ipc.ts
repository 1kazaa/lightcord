...
handleSync(IpcEvents.GET_GIT_HASH, () => LIGHTCORD_GIT_HASH);
...
// Handler pour VencordNative.LIGHTCORD.relaunch() — utilisé par les boutons Restart dans les settings plugins
...
handleSync(IpcEvents.IS_USING_CUSTOM_VENCORD_DIR, () => !!State.store.LIGHTCORDDir);
handle(IpcEvents.SHOW_CUSTOM_VENCORD_DIR, async () => {
    const { LIGHTCORDDir } = State.store;
    if (!LIGHTCORDDir) return;

    const stats = await stat(LIGHTCORDDir);
    if (!stats.isDirectory()) return;

    shell.openPath(LIGHTCORDDir);
});
...
handle(IpcEvents.SELECT_VENCORD_DIR, async (_e, value?: null) => {
    if (value === null) {
        delete State.store.LIGHTCORDDir;
        return "ok";
    }
