...
export async function askToApplySteamLayout(win: BrowserWindow) {
    ...
    const { response } = await dialog.showMessageBox(win, {
        message: `${update ? "Update" : "Apply"} LIGHTCORD Steam Input Layout?`,
        detail: `Would you like to ${update ? "Update" : "Apply"} LIGHTCORD's recommended Steam Deck controller settings?
