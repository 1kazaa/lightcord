...
const options = {
    ...
    repair: {
        type: "boolean",
        short: "r",
        description: "Re-download LIGHTCORD and restart"
    }
...
const extraOptions = {
    ...
    "ozone-platform": {
        hidden: process.platform !== "linux",
        type: "string",
        description: "Whether to run LIGHTCORD in Wayland or X11 (XWayland)",
        options: ["x11", "wayland"]
    }
} satisfies Record<string, Option>;
...
export async function checkCommandLineForRepair() {
    ...
    if (State.store.LIGHTCORDDir) {
        console.error("Cannot repair: using custom LIGHTCORD directory. Remove it in settings first.");
        process.exit(1);
    }

    console.log("Repairing LIGHTCORD...");
...
export function checkCommandLineForHelpOrVersion() {
    ...
    if (version) {
        console.log(`LIGHTCORD v${app.getVersion()}`);
        app.exit(0);
    }

    if (help) {
        const base = stripIndent`
            LIGHTCORD v${app.getVersion()}
...
function checkCommandLineForToggleCommands() {
    ...
    console.error("LIGHTCORD is not running. Toggle commands require a running instance.");
    app.exit(1);
}
...
function checkForSecondInstance() {
    ...
    if (!app.requestSingleInstanceLock({ IS_DEV })) {
        if (IS_DEV) {
            console.log("LIGHTCORD is already running. Quitting previous instance...");
            return;
        } else {
            console.log("LIGHTCORD is already running. Quitting...");
            app.exit(0);
        }
    }
