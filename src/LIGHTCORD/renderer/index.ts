...
import { VoiceActions } from "@LIGHTCORD/types/webpack/common";
import { onceReady } from "@LIGHTCORD/types/webpack";
import { openModal } from "@LIGHTCORD/types/utils";
...
import type SettingsPlugin from "@LIGHTCORD/types/plugins/_core/settings";
import { VesktopSettingsIcon } from "shared/icons";

VesktopLogger.log("read if cute :3");
VesktopLogger.log(`LIGHTCORD v${VesktopNative.app.getVersion()}`);

const { customEntries, customSections } = Vencord.Plugins.plugins.Settings as any as typeof SettingsPlugin;

customEntries.push({
    key: "LIGHTCORD_LIGHTCORD_settings",
    title: "LIGHTCORD Settings",
    Component: SettingsUi,
    Icon: VesktopSettingsIcon
});

customSections.push(() => ({
    section: "LIGHTCORDSettings",
    label: "LIGHTCORD Settings",
    element: SettingsUi,
    className: "vc-LIGHTCORD-settings"
}));
