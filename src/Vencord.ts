...
import "~plugins";
console.log("%c[LIGHTCORD]", "color: #5865f2; font-weight: bold;", "Injection successful! Starting services...");
...
function showGreenUpdateBanner() {
    if (document.getElementById("LIGHTCORD-core-updater-root")) return;

    const banner = document.createElement("div");
    banner.id = "LIGHTCORD-core-updater-root";
    ...
    titleSpan.textContent = "🔔 LIGHTCORD Update Available!";
...
function initTrayIpc() {
    ...
    VencordNative.tray.onCheckUpdates(async () => {
        try {
            ...
            if (isOutdated) {
                showNotice("A LIGHTCORD update is available!", "View Update", () => openSettingsTabModal(UpdaterTab!));
            } else {
    ...
    VencordNative.tray.onRepair(async () => {
        try {
            ...
            UpdateLogger.error("Failed to repair LIGHTCORD", err);
        }
    });
    ...
    if (IS_DEV) {
        ...
        if (pendingPatches.length)
            PMLogger.warn(
                ...
                "You are seeing this warning because this is a Development build of LIGHTCORD.",
