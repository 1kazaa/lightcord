...
const LIGHTCORD_RELEASES_REPO = "LIGHTCORDoff/LIGHTCORD";
const LIGHTCORD_REPO_URL = `https://github.com/${LIGHTCORD_RELEASES_REPO}`;
...
export { LIGHTCORD_REPO_URL };

export async function getCommitsSinceLastSeen(
    repoUrl: string,
): Promise<ChangelogEntry[]> {
    // Toujours utiliser le repo LIGHTCORD, ignorer le repoUrl d'Equicord
    return fetchCommitsBetween(LIGHTCORD_RELEASES_REPO, "HEAD~10", "HEAD").catch(() => []);
}
