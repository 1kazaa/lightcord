...
function useTutorialExists(pluginName: string) {
    ...
    React.useEffect(() => {
        ...
        let cancelled = false;
        fetch(
            `https://raw.githubusercontent.com/LIGHTCORDoff/LIGHTCORD-tutorials/main/videos/${pluginName}.mp4`,
            { method: "HEAD" }
        )
            .then(res => {
                if (!cancelled) {
                    ...
                    window.dispatchEvent(new CustomEvent("LIGHTCORD-tutorial-detected", { detail: { pluginName, found } }));
                }
            })
            .catch(() => {
                if (!cancelled) {
...
export function PluginCard({ plugin, disabled, onRestartNeeded, onMouseEnter, onMouseLeave, isNew }: PluginCardProps) {
    ...
    const openTutorialVideo = (e: React.MouseEvent) => {
        e.stopPropagation();
        const videoUrl = `https://raw.githubusercontent.com/LIGHTCORDoff/LIGHTCORD-tutorials/main/videos/${plugin.name}.mp4`;
        openModal(props => (
