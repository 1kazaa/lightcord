...
export const ExcludedReasons: Record<"web" | "discordDesktop" | "vesktop" | "equibop" | "desktop" | "dev", string> = {
    ...
    dev: "Developer version of LIGHTCORD"
};
...
export default function PluginSettings({ premiumOnly = false }: PluginSettingsProps) {
    ...
    const [searchValue, setSearchValue] = useState({ value: "", status: SearchStatus.LIGHTCORD });
    ...
    React.useEffect(() => {
        ...
        window.addEventListener("LIGHTCORD-tutorial-detected", handler);
        return () => window.removeEventListener("LIGHTCORD-tutorial-detected", handler);
    }, []);
    ...
    const pluginFilter = useCallback((plugin: typeof Plugins[keyof typeof Plugins], newPluginsSet: Set<string> | null) => {
        ...
        switch (status) {
            ...
            case SearchStatus.ENABLED:
                if (!enabled) return false;
                break;
            case SearchStatus.LIGHTCORD:
                if (!PluginMeta[plugin.name].folderName.startsWith("src/LIGHTCORDplugins/")) return false;
                break;
            case SearchStatus.OTHERS:
                if (PluginMeta[plugin.name].folderName.startsWith("src/LIGHTCORDplugins/") || PluginMeta[plugin.name].folderName.startsWith("src/plugins/_")) return false;
    ...
    const { LIGHTCORDPlugins, othersPlugins, requiredPlugins } = useMemo(() => {
        const LIGHTCORDPlugins = [] as JSX.Element[];
        ...
        for (const p of sortedPlugins) {
            ...
            if (isRequired) {
                const tooltipText = p.required || !depMap[p.name]
                    ? "This plugin is required for LIGHTCORD to function."
                ...
                const isLIGHTCORD = folderName.startsWith("src/LIGHTCORDplugins/");
                const card = (
                    <PluginCard
                        onRestartNeeded={handleRestartNeeded}
                        disabled={false}
                ...
                if (isLIGHTCORD) {
                    LIGHTCORDPlugins.push(card);
                } else {
                    othersPlugins.push(card);
                }
            }
        }
        return { LIGHTCORDPlugins, othersPlugins, requiredPlugins };
    ...
    const allPlugins = [...LIGHTCORDPlugins, ...othersPlugins];
    ...
    const LIGHTCORDVisible = LIGHTCORDPlugins.slice(0, Math.min(visibleCount, LIGHTCORDPlugins.length));
    const othersVisible = othersPlugins.slice(0, Math.max(0, visibleCount - LIGHTCORDPlugins.length));
        ...
        <SettingsTab>
                    ...
                    <ErrorBoundary noop>
                        <Select
                            options={[
                                ...
                                { label: "Show LIGHTCORD Plugins", value: SearchStatus.LIGHTCORD },
            ...
            {premiumOnly ? (
                <>
                    ...
                    {LIGHTCORDPlugins.length || othersPlugins.length
                        ? (
                            <div className={cl("grid")}>
                                {[...LIGHTCORDVisible, ...othersVisible].length
                                    ? [...LIGHTCORDVisible, ...othersVisible]
                ...
                <>
                    {LIGHTCORDPlugins.length > 0 && (
                        <>
                            <HeadingTertiary className={Margins.top20}>LIGHTCORD Plugins</HeadingTertiary>
                            <div className={cl("grid")}>
                                {LIGHTCORDVisible}
                            </div>
                        </>
                    ...
                    {LIGHTCORDPlugins.length === 0 && othersPlugins.length === 0 && (
                        <ExcludedPluginsList search={search} />
                    )}
