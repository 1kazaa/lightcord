...
try { _stealthActive = localStorage.getItem("LIGHTCORD_stealthMode") === "1"; } catch { }
...
function persistStealth(v: boolean) {
    try { v ? localStorage.setItem("LIGHTCORD_stealthMode", "1") : localStorage.removeItem("LIGHTCORD_stealthMode"); } catch { }
}

// Éléments NON-React uniquement (titlebar, LIGHTCORDAI nav)
// NE PAS cacher les entrées settings sidebar — l'utilisateur doit pouvoir accéder aux paramètres
const NON_REACT_SELECTORS = [
    "#LIGHTCORD-titlebar-btn",
    "#LIGHTCORD-titlebar-link-style",
    ".nai-nav-item",
];
...
export function syncStealthBodyClass() {
    try { if (_stealthActive) document.body?.classList.add("LIGHTCORD-stealth"); else document.body?.classList.remove("LIGHTCORD-stealth"); } catch { }
...
if (_stealthActive) {
    ...
    try { document.body?.classList.add("LIGHTCORD-stealth"); } catch { }
}
...
try {
    ...
    window.addEventListener("LIGHTCORD-stealth-change", () => {
...
export function _notifyStealthChange() {
    ...
    window.dispatchEvent(new Event("LIGHTCORD-stealth-change"));
}
...
function HeaderBarButtons() {
    ...
    useEffect(() => {
        ...
        window.addEventListener("LIGHTCORD-stealth-change", listener);
        return () => {
            headerBarListeners.delete(listener);
            stealthListeners.delete(listener);
            window.removeEventListener("LIGHTCORD-stealth-change", listener);
        };
    }, []);
