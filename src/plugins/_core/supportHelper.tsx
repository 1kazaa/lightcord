...
import { platformName } from "@LIGHTCORDplugins/equicordHelper/utils";
...
export default definePlugin({
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        // Groupe 2 : Message "LIGHTCORD developers"
        const LIGHTCORDGroupIds = [
        ...
        const isLIGHTCORDGroup = LIGHTCORDGroupIds.includes(userId);

        if (!isOriginalDev && !isLIGHTCORDGroup) return null;
        if (RelationshipStore.isFriend(userId) || isAnyPluginDev(UserStore.getCurrentUser()?.id)) return null;

        const developerText = isOriginalDev ? "Equicord & Vencord plugin developers" : "LIGHTCORD developers";
