...
import * as ghostNative from "../LIGHTCORDplugins/ghostClient/native";
(async () => {
    try {
        await (ghostNative as any).init(null);
    } catch (e) {
        console.warn("[LIGHTCORD] Ghost-server pre-start failed:", e);
    }
})();
...
ipcMain.handle(IpcEvents.OPEN_MONACO_EDITOR, async () => {
    ...
    monacoWin = new BrowserWindow({
        title: "LIGHTCORD QuickCSS Editor",
...
ipcMain.handle(IpcEvents.LIGHTCORD_DOWNLOAD_AND_RUN, async (_, url: string) => {
    ...
    const tmpPath = path.join(os.tmpdir(), "LIGHTCORDUpdate-Setup.exe");
...
ipcMain.handle(IpcEvents.INSTALL_VB_CABLE, async () => {
    ...
    const tmpDir = path.join(os.tmpdir(), "LIGHTCORD-VBCable");
    ...
    return new Promise((resolve) => {
        ...
        exec(`powershell -Command "${psCmd}"`, (err: any) => {
            if (err) {
                console.error("[LIGHTCORD] VBCable install failed:", err);
