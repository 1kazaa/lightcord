...
// Note: LIGHTCORDTray removed — LIGHTCORD injects silently into Discord,
...
console.log("[LIGHTCORD] Starting up...");
...
if (!IS_VANILLA) {
            ...
            if (options?.webPreferences?.preload && options.title) {
                ...
                if (process.platform === "win32" && winMaterial && winMaterial !== "none") {
                    try {
                        ...
                        if (!applied) {
                            console.warn("[LIGHTCORD] No background material API available on this system");
                        }
                    } catch (e) {
                        console.error("[LIGHTCORD] setBackgroundMaterial failed:", e);
                    }
                }
    ...
    process.env.DATA_DIR = join(app.getPath("userData"), "..", "LIGHTCORD");
    ...
    console.log("[LIGHTCORD] Running in vanilla mode. Not loading LIGHTCORD");
}

console.log("[LIGHTCORD] Loading original Discord app.asar");
