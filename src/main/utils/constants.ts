...
export const DATA_DIR = process.env.LIGHTCORD_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "LIGHTCORDData", suffix)
        : join(app.getPath("userData"), "..", "LIGHTCORD", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[LIGHTCORD] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
