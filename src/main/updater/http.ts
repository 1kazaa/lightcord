/*
 * LIGHTCORD — Auto-updater (HTTP / GitHub Releases via ASAR)
...
const RELEASES_REPO = "LIGHTCORDoff/LIGHTCORD";
...
const ZIP_FILE = "LIGHTCORD-dist.zip";
...
async function applyUpdates(): Promise<boolean> {
    ...
    try {
        ...
        // Save zip to temp
        const zipPath = join(app.getPath("temp"), `LIGHTCORD-update-${Date.now()}.zip`);
        ...
        const tmpExtract = join(app.getPath("temp"), `LIGHTCORD-extract-${Date.now()}`);
...
app.on("before-quit", (event) => {
    ...
    console.log("[LIGHTCORD] Applying pending update before quit...");

    const safetyTimeout = setTimeout(() => {
        console.error("[LIGHTCORD] Update on quit timed out — forcing exit.");
    ...
    applyUpdates()
        .then(ok => {
            if (ok) console.log("[LIGHTCORD] Update applied successfully on quit.");
            else    console.warn("[LIGHTCORD] Update on quit returned false.");
        })
        .catch(err => {
            console.error("[LIGHTCORD] Update on quit failed:", err);
