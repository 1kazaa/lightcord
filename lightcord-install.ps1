# ==============================================================================
#  LIGHTCORD — Installeur utilisateur (PowerShell autonome)
#  
#  Ce script fait TOUT automatiquement :
#  1. Télécharge EquilotlCli.exe (outil d'injection graphique)
#  2. Télécharge les fichiers LIGHTCORD compilés depuis GitHub
#  3. Lance l'interface graphique pour choisir votre Discord cible
#  4. Injecte LIGHTCORD dans Discord
#
...
function Write-Banner {
    ...
    Write-Host "  ║          LIGHTCORD  INSTALLER            ║" -ForegroundColor Cyan
...
if (Test-Path $EquilotlExe) {
    ...
    try {
        $head = Invoke-WebRequest -Uri $EquilotlUrl -Method Head -UseBasicParsing `
            -Headers @{ "User-Agent" = "LIGHTCORD-Installer/2.0" }
...
# ── [2/3] Télécharger les fichiers LIGHTCORD ──────────────────────────────────
Write-Step 2 3 "Téléchargement des fichiers LIGHTCORD depuis GitHub..."

try {
    $apiUrl   = "https://api.github.com/repos/$LIGHTCORDRepo/releases/latest"
    $release  = Invoke-RestMethod -Uri $apiUrl -UseBasicParsing `
        -Headers @{ "User-Agent" = "LIGHTCORD-Installer/2.0"; "Accept" = "application/vnd.github.v3+json" }

    $version  = $release.tag_name
    $distAsset = $release.assets | Where-Object { $_.name -eq "LIGHTCORD-dist.zip" } | Select-Object -First 1

    if (-not $distAsset) {
        Write-Fail "Fichier 'LIGHTCORD-dist.zip' introuvable dans la release $version.`n           Contactez le support LIGHTCORD."
    }

    Write-Host "          Version : $version" -ForegroundColor DarkGray
    Write-Host "          Téléchargement en cours..." -ForegroundColor DarkGray

    $zipPath = Join-Path $InstallDir "LIGHTCORD-dist.zip"
    ...
    Write-OK "LIGHTCORD $version prêt à être injecté !"
} catch {
    Write-Fail "Échec du téléchargement LIGHTCORD.`n           Détail : $_"
}
...
Write-Host "          │  Sélectionnez le Discord où injecter LIGHTCORD. │" -ForegroundColor DarkCyan
Write-Host "          └─────────────────────────────────────────────────┘" -ForegroundColor DarkCyan
Write-Host ""
...
Write-Host "  ║  LIGHTCORD installé avec succès !                    ║" -ForegroundColor Green
Write-Host "  ║                                                      ║" -ForegroundColor Green
Write-Host "  ║  → Redémarrez Discord pour appliquer LIGHTCORD.      ║" -ForegroundColor Green
Write-Host "  ╚══════════════════════════════════════════════════════╝" -ForegroundColor Green
Write-Host ""
Write-Host "  Pour désinstaller : exécutez LIGHTCORD-uninstall.bat" -ForegroundColor DarkGray
Write-Host ""
