@echo off
:: ─── LIGHTCORD — Publier une nouvelle release sur GitHub ─────────────────────
...
if "%NOTES%"=="" set NOTES=LIGHTCORD %VERSION%

:: Chemins de sortie
set DIST_DIR=dist\desktop
set OUT_DIR=release\installer
set DIST_ZIP=%OUT_DIR%\LIGHTCORD-dist.zip
set INSTALLER_EXE=%OUT_DIR%\LIGHTCORD-Installer.exe
set VERSION_JSON=%OUT_DIR%\version.json
set DESKTOP_ASAR=dist\desktop.asar

echo.
echo  ╔═══════════════════════════════════════════════════╗
echo  ║    LIGHTCORD — Publication release v%VERSION%
echo  ╚═══════════════════════════════════════════════════╝
echo.
...
:: ── 5. Compiler LIGHTCORD-Installer.exe ──────────────────────────────────────
echo.
echo  [5/8] Compilation de LIGHTCORD-Installer.exe...
...
if not exist "%INSTALLER_EXE%" (
    echo  [ERREUR] LIGHTCORD-Installer.exe introuvable apres compilation.
    pause
    exit /b 1
)

for %%F in ("%INSTALLER_EXE%") do echo  [5/8] LIGHTCORD-Installer.exe cree (%%~zF octets)

:: ── 6. Créer LIGHTCORD-dist.zip ──────────────────────────────────────────────
echo.
echo  [6/8] Creation de LIGHTCORD-dist.zip...
...
powershell -NoProfile -Command "Add-Type -Assembly System.IO.Compression.FileSystem; $src = (Resolve-Path '%DIST_DIR%').Path; $dst = (Join-Path (Resolve-Path 'release\installer').Path 'LIGHTCORD-dist.zip'); [System.IO.Compression.ZipFile]::CreateFromDirectory($src, $dst, [System.IO.Compression.CompressionLevel]::Optimal, $false)"

if not exist "%DIST_ZIP%" (
    echo  [ERREUR] Impossible de creer LIGHTCORD-dist.zip
    pause
    exit /b 1
)

for %%F in ("%DIST_ZIP%") do echo  [6/8] LIGHTCORD-dist.zip cree (%%~zF octets)
...
(
    ...
    echo   "installerUrl": "https://github.com/1kazaa/lightcord/releases/latest/download/LIGHTCORD-Installer.exe",
    echo   "distUrl": "https://github.com/1kazaa/lightcord/releases/latest/download/LIGHTCORD-dist.zip",
    echo   "downloadUrl": "https://github.com/1kazaa/lightcord/releases/latest/download/desktop.asar",
...
gh release create "v%VERSION%" ^
    "%INSTALLER_EXE%#LIGHTCORD-Installer.exe" ^
    "%DIST_ZIP%#LIGHTCORD-dist.zip" ^
    "%DESKTOP_ASAR%#desktop.asar" ^
    "%VERSION_JSON%#version.json" ^
    --repo 1kazaa/lightcord ^
    --title "LIGHTCORD v%VERSION%" ^
    --notes "!NOTES!" ^
    --latest
...
echo.
echo  ╔═══════════════════════════════════════════════════════════════╗
echo  ║  LIGHTCORD v%VERSION% publie avec succes !
echo  ║
echo  ║  Fichiers publies sur GitHub :
echo  ║    LIGHTCORD-Installer.exe    — installeur .exe avec GUI
echo  ║    LIGHTCORD-dist.zip         — JS obfusques (pour l'injec.)
echo  ║    version.json               — metadonnees de version
echo  ║
echo  ║  Les utilisateurs telechargeront LIGHTCORD-Installer.exe
