// LIGHTCORD preload — globalPaths fix + Equicord avec contextBridge
"use strict";
(function () {
    ...
    try {
        ...
        if (rendererJs) {
            webFrame.executeJavaScript(rendererJs).catch(e => {
                console.error("[LIGHTCORD] renderer inject failed:", e?.message);
            });
        }
    } catch (e) {
        console.error("[LIGHTCORD] VencordPreloadGetRendererJs failed:", e);
    }
