...
namespace LIGHTCORDInstaller
{
    ...
    public class LauncherForm : Form
    {
        ...
        private LIGHTCORDBackend _backend;

        public LauncherForm()
        {
            this.Text = "LIGHTCORD Installer";
            this.Size = new Size(740, 620); // Enlarged to prevent text clipping
            this.FormBorderStyle = FormBorderStyle.None;
            this.StartPosition = FormStartPosition.CenterScreen;
            this.BackColor = Color.FromArgb(243, 244, 248); // matching HTML root (light glow theme)
            var iconStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("LIGHTCORDInstaller.icon.ico");
        ...
        {
            var userDataFolder = Path.Combine(Path.GetTempPath(), "LIGHTCORDInstaller_WebView2");
            ...
            )
            {
                ...
                MessageBox.Show(
                    "Microsoft Edge WebView2 Runtime is required to run the LIGHTCORD Installer but was not found on your system.\n\n" +
                    "Please download and install it from:\nhttps://aka.ms/webview2\n\n" +
                    "After installing, restart the LIGHTCORD Installer.",
            ...
            _backend = new LIGHTCORDBackend(this, _webView);
            ...
            using (var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("LIGHTCORDInstaller.index.html"))
            using (var reader = new StreamReader(stream))
            {
                ...
                try {
                    using (var iconStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("LIGHTCORDInstaller.icon.ico"))
                    {
    ...
    public class LIGHTCORDBackend
    {
        ...
        const string GITHUB_REPO = "1kazaa/lightcord";
        const string DIST_ZIP = "LIGHTCORD-dist.zip";

        public LIGHTCORDBackend(LauncherForm form, WebView2 webView)
        {
            _form = form;
            _webView = webView;
            ...
            _distDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "LIGHTCORD", "dist");
        }
        ...
        public async Task<bool> IsInjected(string path)
        {
            return await Task.Run(() => {
                ...
                return Directory.Exists(appDir) && File.Exists(pkgPath) && File.ReadAllText(pkgPath).Contains("\"LIGHTCORD\"");
            });
        }

        /// <summary>
        /// Returns true if a third-party mod (Vencord, Equicord, OpenAsar) is detected
        /// but LIGHTCORD is NOT yet injected.
        /// </summary>
        public async Task<bool> HasThirdPartyMod(string path)
        {
            return await Task.Run(() => {
                ...
                // Already LIGHTCORD → not a third-party mod
                if (content.Contains("\"LIGHTCORD\"")) return false;
        ...
        private async Task EnsureDistAsync()
        {
            ...
            _http.DefaultRequestHeaders.Add("User-Agent", "LIGHTCORD-Installer/2.0");
            ...
            var tmpZip = Path.Combine(Path.GetTempPath(), "LIGHTCORD-dist.zip");
                ...
                using (var fs = new FileStream(tmpZip, FileMode.Create, FileAccess.Write, FileShare.None, 81920, true))
                {
                    ...
                    while ((read = await contentStream.ReadAsync(buffer, 0, buffer.Length)) > 0)
                    {
                        ...
                        if (percent - lastReportedPercent >= 0.5 || percent >= 100.0)
                        {
                            ...
                            SetProgress(overallPercent, "Downloading LIGHTCORD...", readMB, totalMB);
                        }
                    }
                }
            }
        ...
        private void DoInject(string resPath)
        {
            ...
            {
                // Déjà LIGHTCORD → on réinjecte proprement sans bail-out partiel
            ...
            SetProgress(92, "Configuring LIGHTCORD loader...");
            ...
            // Créer le backup LIGHTCORD si app.asar existe et qu'on n'a pas encore de backup
        ...
        private void CleanModulePatches(string resPath)
        {
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine($"[LIGHTCORD] CleanModulePatches warning: {ex.Message}");
            }
        }
        ...
        private void DoUninject(string resPath)
        {
            ...
            if (Directory.Exists(appDir))
            {
                ...
                if (File.Exists(pkg) && File.ReadAllText(pkg).Contains("\"LIGHTCORD\""))
                {
            ...
            // 3. Clean up LIGHTCORD-specific assets (folders we added)
        ...
        private void WriteLoader(string appDir)
        {
            ...
            // _distDir = %LOCALAPPDATA%\LIGHTCORD\dist\
            ...
            File.WriteAllText(Path.Combine(appDir, "package.json"), "{\"name\":\"LIGHTCORD\",\"main\":\"index.js\"}");
            File.WriteAllText(Path.Combine(appDir, "index.js"),
                ...
                $"if (!fs.existsSync(patcherPath)) throw new Error('[LIGHTCORD] patcher.js not found. Expected at: ' + primary);\n" +
                $"require(patcherPath);\n"
            );
        }
