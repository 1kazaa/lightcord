...
// ── CRITIQUE : userData = dossier LIGHTCORD pour les settings/plugins
...
const LIGHTCORDData = path.join(app.getPath("appData"), "LIGHTCORD");
app.setPath("userData", LIGHTCORDData);
...
// pour le token Discord stable. Quand LIGHTCORD charge ce cache, il lit des permissions
...
// AppUserModelId unique — Windows reconnaît LIGHTCORD comme app séparée de Discord
...
app.once("ready", () => {
    try {
        ...
        //     Seuls les modules vraiment inutiles pour LIGHTCORD sont bloqués.
        ...
        app.once("browser-window-created", () => {
            try {
                ...
                ses.webRequest.onBeforeRequest(
                    ...
                    (details, callback) => {
                        ...
                        if (isBlocked) {
                            ...
                            console.log("[LIGHTCORD] Module bloqué (inutile pour LIGHTCORD):", url.split("/").slice(-2).join("/"));
                ...
                console.log("[LIGHTCORD] Filtre modules 403 activé ✓");
            } catch (e) {
                console.warn("[LIGHTCORD] Impossible d'activer le filtre modules:", e.message);
            }
        });
    } catch (e) {
        console.warn("[LIGHTCORD] FIX modules 403 failed:", e.message);
    }
});
...
try {
    const lsPath = path.join(LIGHTCORDData, "Local Storage", "leveldb");
    if (fs.existsSync(lsPath)) {
        ...
        if (corrupted) {
            console.warn("[LIGHTCORD] LevelDB localStorage corrompu détecté — réparation...");
            try { fs.rmSync(lsPath, { recursive: true, force: true }); } catch { }
            console.warn("[LIGHTCORD] LevelDB supprimé — les données localStorage seront récréées");
        }
    }
} catch (e) { console.warn("[LIGHTCORD] LevelDB check failed:", e.message); }

// Modules bundlés dans LIGHTCORD-dist/modules/
...
try {
    ...
    if (entries.length > 0) {
        ...
        console.log("[LIGHTCORD] Modules natifs Discord détectés:", discordNativeModulesPath);
    }
} catch (e) {
    console.warn("[LIGHTCORD] Impossible de détecter les modules natifs Discord:", e.message);
}
...
// Priorité aux modules bundlés (portables, dans LIGHTCORD-dist/modules/)
...
if (discordNativeModulesPath) {
    ...
    } catch (e) { console.warn("[LIGHTCORD] Erreur lors du scan des modules natifs:", e.message); }
}
try {
...
console.log("[LIGHTCORD] mainAppDirname:", global.mainAppDirname);
...
try {
    ...
    if (fs.existsSync(nativeModulesDir) && !buildInfo.localModulesRoot) {
        ...
        console.log("[LIGHTCORD] build_info.json patché → localModulesRoot:", nativeModulesDir);
    }
} catch (e) {
    console.warn("[LIGHTCORD] Impossible de patcher build_info.json:", e.message);
}
