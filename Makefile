...
RC_OBJ = $(BUILD_DIR)/LIGHTCORD_rc.o
...
all: $(BUILD_DIR)/LIGHTCORD $(BUILD_DIR)/updater.exe
...
$(BUILD_DIR)/LIGHTCORD_rc.o: LIGHTCORD.rc assets/LIGHTCORD.ico
	mkdir -p $(BUILD_DIR)
	windres LIGHTCORD.rc -o $@

$(BUILD_DIR)/%.o: %.c
	mkdir -p $(dir $@)
	$(CC) -c -o $@ $(FLAGS) $< -Wall -Wno-unused-function -Wno-misleading-indentation $(OPTS)

$(BUILD_DIR)/LIGHTCORD: $(OBJS) $(BUILD_DIR)/resources.o $(RC_OBJ) $(BUILD_DIR)/themes/ $(BUILD_DIR)/sounds/
...
uninstall:
	rm -f $(PREFIX)/share/applications/LIGHTCORD.desktop
	rm -f $(PREFIX)/share/pixmaps/LIGHTCORD.png
	rm -f $(PREFIX)/share/pixmaps/LIGHTCORD.svg
	rm -f $(PREFIX)/bin/LIGHTCORD

install: uninstall
	cp LIGHTCORD.desktop $(PREFIX)/share/applications/LIGHTCORD.desktop
	cp assets/icon.svg $(PREFIX)/share/pixmaps/LIGHTCORD.svg
	cp $(BUILD_DIR)/LIGHTCORD $(PREFIX)/bin/LIGHTCORD

run: all
	cp assets/LIGHTCORD.gschema.xml $(BUILD_DIR)/assets/LIGHTCORD.gschema.xml
