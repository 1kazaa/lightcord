...
// ─── Configuration de Build LIGHTCORD ─────────────────────────────────────────

function killLIGHTCORD() {
    const releaseDir = join(__dirname, "release", "LIGHTCORD-dist");
...
function buildLIGHTCORDFromDiscord(discordApp) {
    ...
    console.log("[LIGHTCORD] Préparation de _app.asar...");
...
app.setPath("userData", path.join(app.getPath("appData"), "LIGHTCORD"));
...
`);
    ...
    writeFileSync(join(outApp, "package.json"), JSON.stringify({ name: "LIGHTCORD", main: "index.js", version: "1.18.0" }, null, 2));
...
`);
    ...
    const LIGHTCORDPreload = join(__dirname, "LIGHTCORD-preload.js");
    if (existsSync(LIGHTCORDPreload)) {
        cpSync(LIGHTCORDPreload, join(outDist, "preload.js"));
    }
    ...
    const iconSrc = join(__dirname, "LIGHTCORD.ico");
    if (existsSync(iconSrc)) {
        ...
        try {
            ...
            if (existsSync(rcedit)) {
                execSync(`"${rcedit}" "${discordExe}" --set-icon "${iconSrc}" --set-version-string "ProductName" "LIGHTCORD" --set-version-string "FileDescription" "LIGHTCORD"`, { stdio: "ignore" });
            }
        } catch (e) { }
    }

    console.log(`[LIGHTCORD] Build terminé -> ${outDir}`);
}
...
buildLIGHTCORDFromDiscord(discord);

module.exports = {
    appId: "com.LIGHTCORD.app",
    productName: "LIGHTCORD",
    copyright: "Copyright 2026 LIGHTCORD",
