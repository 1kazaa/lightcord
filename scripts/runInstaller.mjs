/*
 * LIGHTCORD — Installer via EquilotlCli
 * Télécharge EquilotlCli.exe depuis les releases Equicord et le lance
 * avec les variables d'environnement pointant vers les fichiers LIGHTCORD.
 *
 * L'exe affiche une interface graphique permettant de choisir le Discord cible.
 *
 * Usage:
 *   pnpm inject    → installe LIGHTCORD dans le Discord choisi
 *   pnpm uninject  → désinstalle LIGHTCORD du Discord choisi
...
// EquilotlCli est l'installeur graphique d'Equicord — on le réutilise pour LIGHTCORD
...
async function ensureBinary() {
    ...
    if (existsSync(outputFile)) {
        console.log("[LIGHTCORD] Installeur déjà présent, utilisation locale.");
        return outputFile;
    }

    console.log("[LIGHTCORD] Téléchargement de l'installeur (" + filename + ")...");

    const res = await fetch(BASE_URL + filename, {
        headers: {
            "User-Agent": "LIGHTCORD (https://github.com/LIGHTCORDoff/LIGHTCORD)"
        }
    });
    ...
    if (process.platform === "darwin") {
        console.log("[LIGHTCORD] Sauvegarde du zip...");
        const zip = new Uint8Array(await res.arrayBuffer());
        writeFileSync(downloadName, zip);

        console.log("[LIGHTCORD] Extraction du bundle...");
        execSync(`ditto -x -k '${downloadName}' '${FILE_DIR}'`);

        console.log("[LIGHTCORD] Suppression de la quarantaine macOS...");
        const logAndRun = cmd => {
    ...
    console.log("[LIGHTCORD] Installeur téléchargé avec succès !");
    return outputFile;
}
...
function checkBuild() {
    ...
    if (!existsSync(patcherPath)) {
        console.error("\x1b[31m[LIGHTCORD] dist/desktop/patcher.js introuvable !\x1b[0m");
...
function cleanOldLIGHTCORD() {
    console.log("[LIGHTCORD] Recherche et nettoyage automatique des anciennes installations...");
    ...
    for (const resourcesDir of candidates) {
        ...
        try {
            ...
            // 1. Supprimer le dossier app/ s'il a été créé par l'ancien LIGHTCORD
            if (existsSync(appDirPath)) {
                ...
                if (shouldDelete) {
                    console.log(`[LIGHTCORD] Suppression de l'ancien dossier app/ dans : ${resourcesDir}`);
            ...
            if (existsSync(backupPath)) {
                ...
                if (isAsarDir) {
                    console.log(`[LIGHTCORD] Suppression du dossier app.asar temporaire dans : ${resourcesDir}`);
                    rmSync(appAsarPath, { recursive: true, force: true });
                }

                if (!existsSync(appAsarPath) || isAsarDir) {
                    console.log(`[LIGHTCORD] Restauration _app.asar -> app.asar dans : ${resourcesDir}`);
                    ...
                    console.log(`[LIGHTCORD] Nettoyage du backup _app.asar obsolète dans : ${resourcesDir}`);
                    rmSync(backupPath, { force: true });
                    cleanedAny = true;
                }
            }
        } catch (e) {
            console.error(`[LIGHTCORD] Erreur lors du nettoyage de ${resourcesDir} :`, e.message);
        }
    }

    if (cleanedAny) {
        console.log("[LIGHTCORD] Nettoyage des anciennes installations terminé avec succès !");
    } else {
        console.log("[LIGHTCORD] Aucune ancienne installation à nettoyer.");
    }
}

// ── Main ─────────────────────────────────────────────────────────────────────
// On nettoie d'abord les anciennes traces de LIGHTCORD pour éviter tout conflit ou blocage
cleanOldLIGHTCORD();
...
if (isInstall) {
    console.log("[LIGHTCORD] Nettoyage des installations précédentes (Vencord/Equicord/LIGHTCORD)...");
    try {
        ...
        console.log("[LIGHTCORD] Nettoyage terminé.");
    } catch {
        console.log("[LIGHTCORD] Aucun mod précédent à nettoyer ou échec du nettoyage.");
    }
}

console.log("[LIGHTCORD] Lancement de l'injection...");
...
try {
    execFileSync(installerBin, args, {
        ...
        env: {
            ...
            EQUICORD_DIRECTORY: join(BASE_DIR, "dist", "desktop"),
            EQUICORD_DEV_INSTALL: "1",
            LIGHTCORD_DIRECTORY: join(BASE_DIR, "dist", "desktop")
        }
    });
} catch {
    console.error("[LIGHTCORD] Erreur lors de l'injection.");
    process.exit(1);
}
