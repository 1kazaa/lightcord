/*
 * LIGHTCORD — Local un-injector for Discord Desktop
...
function findAllDiscordResources() {
    ...
    // Filtrer uniquement les paths avec une injection LIGHTCORD présente
...
function uninject(resourcesDir) {
    ...
    // Vérifier que le dossier app/ a bien été créé par LIGHTCORD
    if (existsSync(appDirPath)) {
        try {
            const pkg = JSON.parse(readFileSync(join(appDirPath, "package.json"), "utf-8"));
            if (pkg.name !== "LIGHTCORD") {
                console.warn(`\x1b[33m[LIGHTCORD] Le dossier app/ existe mais n'a pas été créé par LIGHTCORD (name: "${pkg.name}").\x1b[0m`);
                console.warn("\x1b[33m            Abandon pour éviter de casser un autre mod.\x1b[0m");
                return false;
            }
        } catch { }

        console.log("[LIGHTCORD] Suppression du dossier app/ injecté...");
        rmSync(appDirPath, { recursive: true, force: true });
    } else {
        console.log("\x1b[33m[LIGHTCORD] Aucun dossier app/ injecté trouvé.\x1b[0m");
    }

    // Restaurer le backup
    if (existsSync(backupPath) && !existsSync(appAsarPath)) {
        console.log("[LIGHTCORD] Restauration _app.asar → app.asar...");
        renameSync(backupPath, appAsarPath);
    } else if (existsSync(backupPath) && existsSync(appAsarPath)) {
        console.log("[LIGHTCORD] app.asar déjà présent, nettoyage du backup...");
        rmSync(backupPath, { force: true });
    }

    console.log(`\x1b[32m[LIGHTCORD] Désinjection réussie depuis : ${resourcesDir}\x1b[0m`);
...
if (allResources.length === 0) {
    console.error("\x1b[31m[LIGHTCORD] Aucune installation Discord avec LIGHTCORD injecté trouvée.\x1b[0m");
    console.error("\x1b[33m           Assurez-vous que LIGHTCORD a bien été injecté via 'pnpm inject'.\x1b[0m");
    process.exit(1);
}

let uninjectCount = 0;
for (const res of allResources) {
    console.log(`\n[LIGHTCORD] Trouvé : ${res}`);
    if (uninject(res)) uninjectCount++;
}

if (uninjectCount === 0) {
    console.error("\x1b[31m[LIGHTCORD] Aucune désinjection réussie.\x1b[0m");
    process.exit(1);
}

console.log(`\n\x1b[32m[LIGHTCORD] ${uninjectCount}/${allResources.length} désinjection(s) réussie(s).\x1b[0m`);
