/*
 * LIGHTCORD — Local injector for Discord Desktop
 * Injecte LIGHTCORD dans une installation Discord existante en :
 * 1. Trouvant le répertoire resources de Discord
 * 2. Renommant app.asar → _app.asar (backup)
 * 3. Créant un dossier app/ avec un loader qui require le patcher.js de LIGHTCORD
 *
...
function inject(resourcesDir) {
    ...
    if (existsSync(appDirPath) && existsSync(join(appDirPath, "package.json"))) {
        try {
            ...
            if (pkg.name === "LIGHTCORD") {
                console.log("\x1b[33m[LIGHTCORD] Déjà injecté ! Utilisez 'pnpm uninject' d'abord pour réinjecter.\x1b[0m");
                return false;
            }
        } catch { }
    }
    ...
    if (existsSync(appAsarPath) && !existsSync(backupPath)) {
        ...
        if (isDir) {
            console.warn("\x1b[33m[LIGHTCORD] app.asar est un dossier — un autre mod est peut-être installé.\x1b[0m");
            console.warn("\x1b[33m            Abandon. Utilisez 'pnpm uninject' pour nettoyer d'abord.\x1b[0m");
            return false;
        }
        console.log("[LIGHTCORD] Sauvegarde app.asar → _app.asar...");
        renameSync(appAsarPath, backupPath);
    } else if (!existsSync(backupPath)) {
        console.error("\x1b[31m[LIGHTCORD] Aucun app.asar ou _app.asar trouvé dans resources !\x1b[0m");
        return false;
    }
    ...
    if (existsSync(appAsarPath)) {
        try {
            ...
            console.error(`\x1b[31m[LIGHTCORD] Impossible de supprimer l'ancien app.asar : ${e.message}\x1b[0m`);
            return false;
        }
    }
    ...
    // Le loader require simplement le patcher LIGHTCORD depuis dist/
    const patcherPath = join(DIST_DIR, "patcher.js").replace(/\\/g, "\\\\");
    writeFileSync(join(appDirPath, "index.js"),
        `// LIGHTCORD Injector — auto-generated, do not edit\n"use strict";\nrequire("${patcherPath}");\n`
    );

    console.log(`\x1b[32m[LIGHTCORD] Injecté avec succès dans : ${resourcesDir}\x1b[0m`);
    console.log(`\x1b[32m[LIGHTCORD] Répertoire LIGHTCORD dist : ${DIST_DIR}\x1b[0m`);
    console.log("\x1b[36m[LIGHTCORD] Redémarrez Discord pour appliquer les changements.\x1b[0m");
    return true;
}
...
if (allResources.length === 0) {
    console.error("\x1b[31m[LIGHTCORD] Aucune installation Discord trouvée !\x1b[0m");
...
if (allResources.length === 1) {
    ...
    console.log(`[LIGHTCORD] Discord trouvé : ${allResources[0]}`);
    inject(allResources[0]);
} else {
    // Plusieurs Discord trouvés : injecter dans tous
    console.log(`[LIGHTCORD] ${allResources.length} installations Discord trouvées :`);
    let injectedCount = 0;
    for (const res of allResources) {
        console.log(`\n  → ${res}`);
        if (inject(res)) injectedCount++;
    }
    console.log(`\n\x1b[32m[LIGHTCORD] ${injectedCount}/${allResources.length} injection(s) réussie(s).\x1b[0m`);
}
