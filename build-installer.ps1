# build-installer.ps1 - Compile LIGHTCORD-Installer.exe
...
$OutExe = Join-Path $OutDir "LIGHTCORD-Installer.exe"

Write-Host ""
Write-Host "  [LIGHTCORD] Compiling installer..." -ForegroundColor Cyan
...
if ($hasDotnet) {
    ...
    & dotnet publish "$SrcDir\LIGHTCORDInstaller.csproj" `
        -c Release `
        -o $OutDir `
        --nologo `
        -v quiet `
    ...
    $built = Get-ChildItem $OutDir -Recurse -Filter "LIGHTCORD-Installer.exe" -ErrorAction SilentlyContinue | Select-Object -First 1
...
# Method 2: csc.exe (.NET Framework - always present on Windows)
    ...
    $ico    = Join-Path $Root "LIGHTCORD.ico"
...
if (Test-Path $OutExe) {
    ...
    Write-Host ""
    Write-Host "  OK  LIGHTCORD-Installer.exe compiled ($size KB)" -ForegroundColor Green
    Write-Host "    -> $OutExe" -ForegroundColor DarkGray
    Write-Host ""
} else {
    Write-Host "  [ERROR] LIGHTCORD-Installer.exe not found after compilation." -ForegroundColor Red
    exit 1
}
